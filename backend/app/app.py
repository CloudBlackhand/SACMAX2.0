...
async def process_excel_file(file: UploadFile = File(...)):
    """Processar arquivo Excel e retornar dados estruturados"""
    try:
        ...
        if excel_service:
            try:
                ...
                logger.error(f"Erro no ExcelService: {e}")
                return {
                    "success": False,
            ...
            try:
                ...
                # Processar primeira aba para preview
                df = pd.read_excel(str(file_path), sheet_name=sheets[0])
                ...
                rows = df.head(100).values.tolist()  # Primeiras 100 linhas para preview
                ...
                logger.error(f"Erro ao processar Excel diretamente: {e}")
                return {
                    "success": False,
                    "error": f"Erro ao processar Excel: {str(e)}"
                }
...
@app.post("/api/excel/import-produtividade")
async def import_produtividade_excel(file: UploadFile = File(...)):
    """Importar dados da aba PRODUTIVIDADE para o banco"""
    try:
        if not file.filename.endswith(('.xlsx', '.xls')):
            raise HTTPException(status_code=400, detail="Apenas arquivos Excel são aceitos")
        ...
        # Processar especificamente a aba PRODUTIVIDADE
        try:
            from excel_to_database import ExcelToDatabaseConverter
            ...
            # Processar aba PRODUTIVIDADE
            excel_file = pd.ExcelFile(str(file_path))
            result = converter.process_produtividade_sheet(excel_file)
            ...
            if result["success"]:
                return {
                    "success": True,
                    "message": f"✅ {result['records_processed']} registros importados com sucesso!",
                    "data": result
                }
            else:
                return {
            ...
            logger.error(f"Erro ao processar PRODUTIVIDADE: {e}")
            return {
                "success": False,
                "error": f"Erro ao processar aba PRODUTIVIDADE: {str(e)}"
            }
            
    except Exception as e:
        logger.error(f"Erro no import PRODUTIVIDADE: {e}")
        raise HTTPException(status_code=500, detail=str(e))
