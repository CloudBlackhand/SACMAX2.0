...
class ExcelModule {
            ...
            <div class="module-container fade-in">
                ...
                <div class="card" id="processing-options" style="display: none;">
                    ...
                    <div class="grid grid-2">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-input" id="import-type">
                                <option value="preview">Preview dos Dados</option>
                                <option value="produtividade">Importar PRODUTIVIDADE</option>
                                <option value="contacts">Importar Contatos</option>
                            </select>
                        </div>
                    ...
                    <div class="processing-actions">
                        ...
                        <button class="btn btn-success" id="import-produtividade-btn" onclick="excelModule.importProdutividade()" style="display: none;">
    ...
    populateProcessingOptions() {
        ...
        // Verificar se tem aba PRODUTIVIDADE e mostrar botão específico
        const importTypeSelect = document.getElementById('import-type');
        const importProdutividadeBtn = document.getElementById('import-produtividade-btn');
        
        if (importTypeSelect && importProdutividadeBtn) {
            importTypeSelect.addEventListener('change', () => {
                if (importTypeSelect.value === 'produtividade') {
                    importProdutividadeBtn.style.display = 'inline-flex';
                } else {
                    importProdutividadeBtn.style.display = 'none';
                }
            });
            
            // Verificar se tem aba PRODUTIVIDADE
            const hasProdutividade = this.processedData.sheets.some(sheet => 
                sheet.toLowerCase().includes('produtividade')
            );
            
            if (hasProdutividade) {
                importTypeSelect.value = 'produtividade';
                importProdutividadeBtn.style.display = 'inline-flex';
            }
        }
    }
    ...
    async importProdutividade() {
        ...
        try {
            ...
            // Chamar API de importação PRODUTIVIDADE
            const response = await fetch(`${SacsMaxConfig.backend.current}/api/excel/import-produtividade`, {
                method: 'POST',
                body: formData
            });
            ...
            if (result.success) {
                ...
                this.addToHistory({
                    filename: this.uploadedFile.name,
                    records: result.data.records_processed,
                    status: 'success',
                    statusText: 'PRODUTIVIDADE Importada'
                });
                
                // Atualizar contador de registros
                const previewCount = document.getElementById('preview-count');
                if (previewCount) {
                    previewCount.textContent = result.data.records_processed;
                }
            ...
            this.showError('Erro ao importar PRODUTIVIDADE: ' + error.message);
